...
>
    ...
    <xsl:template match="gmd:MD_Metadata/gmd:language/gco:CharacterString">
        <xsl:call-template name="element">
            <xsl:with-param name="title">Metadata taal
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="gmd:MD_Metadata/gmd:characterSet/gmd:MD_CharacterSetCode">
        <xsl:call-template name="element">
            <xsl:with-param name="title">Metadata karakterset
            </xsl:with-param>
            <xsl:with-param name="picklist">picklist_MD_CharacterSetCode
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="gmd:MD_Metadata/gmd:dataQualityInfo//gmd:level/gmd:MD_ScopeCode">
        <xsl:call-template name="element">
            <xsl:with-param name="title">Metadata hiërarchieniveau
            </xsl:with-param>
            <xsl:with-param name="picklist">picklist_MD_ScopeCode
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="gmd:MD_Metadata/gmd:hierarchyLevelName/gco:CharacterString">
        <xsl:call-template name="element">
            <xsl:with-param name="title">Beschrijving hiërarchisch niveau
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="gmd:MD_Metadata/gmd:contact">
        <div class="section">
            <xsl:call-template name="section-title">
                <xsl:with-param name="title">Organisatie verantwoordelijk voor metadata
                </xsl:with-param>
                <xsl:with-param name="repeatable" select="'true'"/>
                <xsl:with-param name="section-path">/gmd:MD_Metadata/gmd:contact[
                    <xsl:value-of select="position()"/>]/gmd:CI_ResponsibleParty
                </xsl:with-param>
            </xsl:call-template>
    ...
    <xsl:template match="gmd:MD_Metadata/gmd:dateStamp/gco:Date">
        <xsl:call-template name="element">
            <xsl:with-param name="title">Metadata datum
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="gmd:metadataStandardName/gco:CharacterString">
        <xsl:call-template name="element">
            <xsl:with-param name="title">Metadatastandaard naam
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="gmd:metadataStandardVersion/gco:CharacterString">
        <xsl:call-template name="element">
            <xsl:with-param name="title">Versie metadatastandaard naam
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="gmd:code/gco:CharacterString">
        <xsl:call-template name="element">
            <xsl:with-param name="title">Code referentiesysteem
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="gmd:codeSpace/gco:CharacterString">
        <xsl:call-template name="element">
            <xsl:with-param name="title">Verantwoordelijke organisatie voor namespace referentiesysteemm
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="gmd:title/gco:CharacterString">
        <xsl:call-template name="element">
            <xsl:with-param name="title">Dataset titel
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="gmd:MD_Metadata/gmd:identificationInfo/gmd:MD_DataIdentification/gmd:citation/gmd:CI_Citation/gmd:date">
        <div class="section">
            <xsl:call-template name="section-title">
                <xsl:with-param name="title">Dataset referentie
                </xsl:with-param>
                <xsl:with-param name="repeatable" select="'true'"/>
                <xsl:with-param name="section-path">/gmd:MD_Metadata/gmd:identificationInfo/gmd:MD_DataIdentification/gmd:citation/gmd:CI_Citation/gmd:date[
                    <xsl:value-of select="position()"/>]/gmd:CI_Date
                </xsl:with-param>
            </xsl:call-template>
            <div class="section-content">
    ...
    <xsl:template match="gmd:date/gco:Date">
        <xsl:call-template name="element">
            <xsl:with-param name="title">Dataset referentie datum
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="gmd:dateType/gmd:CI_DateTypeCode">
        <xsl:call-template name="element">
            <xsl:with-param name="title">Creatie-, publicatie-, of wijzigingsdatum
            </xsl:with-param>
            <xsl:with-param name="picklist">picklist_CI_DateTypeCode
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="gmd:abstract/gco:CharacterString">
        <xsl:call-template name="element">
            <xsl:with-param name="title">Samenvatting
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="gmd:status/gmd:MD_ProgressCode">
        <xsl:call-template name="element">
            <xsl:with-param name="title">Status
            </xsl:with-param>
            <xsl:with-param name="picklist">picklist_MD_ProgressCode
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="gmd:MD_Metadata/gmd:identificationInfo/gmd:MD_DataIdentification/gmd:pointOfContact">
        <div class="section">
            <xsl:call-template name="section-title">
                <xsl:with-param name="title">Dataset verantwoordelijke organisatie
                </xsl:with-param>
                <xsl:with-param name="repeatable" select="'true'"/>
                <xsl:with-param name="section-path">/gmd:MD_Metadata/gmd:identificationInfo/gmd:MD_DataIdentification/gmd:pointOfContact[
                    <xsl:value-of select="position()"/>]/gmd:CI_ResponsibleParty
                </xsl:with-param>
            </xsl:call-template>
            <div class="section-content">
    ...
    <xsl:template match="/gmd:MD_Metadata/gmd:identificationInfo/gmd:MD_DataIdentification/gmd:descriptiveKeywords/gmd:MD_Keywords/gmd:keyword/gco:CharacterString">
        <xsl:call-template name="element">
            <xsl:with-param name="title">Trefwoorden
            </xsl:with-param>
            <xsl:with-param name="repeatable" select="'true'"/>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="gmd:useLimitation/gco:CharacterString">
        <xsl:call-template name="element">
            <xsl:with-param name="title">Gebruiksbeperkingen
            </xsl:with-param>
            <xsl:with-param name="repeatable" select="'true'"/>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="gmd:accessContraints/gmd:MD_RestrictionCode">
        <xsl:call-template name="element">
            <xsl:with-param name="title">(juridische) toegangsrestricties
            </xsl:with-param>
            <xsl:with-param name="picklist">picklist_MD_RestrictionCode
            </xsl:with-param>
            <xsl:with-param name="repeatable" select="'true'"/>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="gmd:spatialRepresentationType/gmd:MD_SpatialRepresentationTypeCode">
        <xsl:call-template name="element">
            <xsl:with-param name="title">Ruimtelijk schema
            </xsl:with-param>
            <xsl:with-param name="picklist">picklist_MD_SpatialRepresentationTypeCode
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="gmd:denominator/gco:Integer">
        <xsl:call-template name="element">
            <xsl:with-param name="title">Toepassingsschaal
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="gmd:identificationInfo//gmd:language/gco:CharacterString">
        <xsl:call-template name="element">
            <xsl:with-param name="title">Dataset taal
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="/gmd:MD_Metadata/gmd:identificationInfo/gmd:MD_DataIdentification/gmd:characterSet/gmd:MD_CharacterSetCode">
        <xsl:call-template name="element">
            <xsl:with-param name="title">Dataset karakterset
            </xsl:with-param>
            <xsl:with-param name="picklist">picklist_MD_CharacterSetCode
            </xsl:with-param>
        </xsl:call-template>
    ...
    <xsl:template match="/gmd:MD_Metadata/gmd:identificationInfo/gmd:MD_DataIdentification/gmd:topicCategory/gmd:MD_TopicCategoryCode">
        <xsl:call-template name="element">
            <xsl:with-param name="title">Thema's
            </xsl:with-param>
            <xsl:with-param name="picklist">picklist_MD_TopicCategoryCode
            </xsl:with-param>
            <xsl:with-param name="repeatable" select="'true'"/>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="gmd:identificationInfo//gmd:extent//gmd:EX_GeographicBoundingBox/gmd:westBoundLongitude/gco:Decimal">
        <xsl:call-template name="element">
            <xsl:with-param name="title">Minimum x
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="gmd:identificationInfo//gmd:extent//gmd:EX_GeographicBoundingBox/gmd:eastBoundLongitude/gco:Decimal">
        <xsl:call-template name="element">
            <xsl:with-param name="title">Maximum x
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="gmd:identificationInfo//gmd:extent//gmd:EX_GeographicBoundingBox/gmd:southBoundLatitude/gco:Decimal">
        <xsl:call-template name="element">
            <xsl:with-param name="title">Minimum y
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="gmd:identificationInfo//gmd:extent//gmd:EX_GeographicBoundingBox/gmd:northBoundLatitude/gco:Decimal">
        <xsl:call-template name="element">
            <xsl:with-param name="title">Maximum y
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="gmd:identificationInfo//gmd:extent//gmd:EX_TemporalExtent/gmd:extent/gmd:TimeInstant/gmd:timePosition[@gmd:indeterminatePosition='after']">
        <xsl:call-template name="element">
            <xsl:with-param name="title">Begindatum
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    <xsl:template match="gmd:identificationInfo//gmd:extent//gmd:EX_TemporalExtent/gmd:extent/gmd:TimeInstant/gmd:timePosition[@gmd:indeterminatePosition='before']">
        <xsl:call-template name="element">
            <xsl:with-param name="title">Einddatum
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="gmd:MD_Metadata/gmd:distributionInfo//gmd:distributor">
        <div class="section">
            <xsl:call-template name="section-title">
                <xsl:with-param name="title">Distribuerende organisatie
                </xsl:with-param>
                <xsl:with-param name="repeatable" select="'true'"/>
                <xsl:with-param name="section-path">/gmd:MD_Metadata/gmd:distributionInfo/gmd:MD_Distribution/gmd:distributor[
                    <xsl:value-of select="position()"/>]/gmd:MD_Distributor/gmd:distributorContact/gmd:CI_ResponsibleParty
                </xsl:with-param>
            </xsl:call-template>
            <div class="section-content">
    ...
    <xsl:template match="gmd:lineage//gmd:statement/gco:CharacterString">
        <xsl:call-template name="element">
            <xsl:with-param name="title">Algemene beschrijving herkomst
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="gmd:organisationName/gco:CharacterString">
        <xsl:call-template name="element">
            <xsl:with-param name="title">Naam organisatie
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    <!-- ISO nr. 379 -->
    <xsl:template match="gmd:role/gmd:CI_RoleCode">
        <xsl:call-template name="element">
            <xsl:with-param name="title">Rol organisatie
            </xsl:with-param>
            <xsl:with-param name="picklist">picklist_CI_RoleCode
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    <!-- ISO nr. 397 -->
    <xsl:template name="link">
        <xsl:call-template name="element">
            <xsl:with-param name="title">URL organisatie
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    <!-- ISO nr. 397 -->
    ...
    <xsl:template name="omgrenzendeRechthoek">
        <div class="section">
            <xsl:call-template name="section-title">
                <xsl:with-param name="title">Omgrenzende rechthoek
                </xsl:with-param>
            </xsl:call-template>
            <div class="section-content">
    ...
    <xsl:template name="temporeleDekking">
        <div class="section">
            <xsl:call-template name="section-title">
                <xsl:with-param name="title">Temporele dekking
                </xsl:with-param>
            </xsl:call-template>
            <div class="section-content">
                <xsl:apply-templates select="/gmd:MD_Metadata/gmd:identificationInfo//gmd:extent//gmd:EX_TemporalExtent/gmd:extent/gmd:TimeInstant/gmd:timePosition[@gmd:indeterminatePosition='after']"/>
                <xsl:apply-templates select="/gmd:MD_Metadata/gmd:identificationInfo//gmd:extent//gmd:EX_TemporalExtent/gmd:extent/gmd:TimeInstant/gmd:timePosition[@gmd:indeterminatePosition='before']"/>
            </div>
        </div>
    </xsl:template>
