...
<xsl:stylesheet version="1.0" xmlns="http://www.w3.org/1999/xhtml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:variable name="PLUS_IMAGE">
		<xsl:value-of select="$basePath"/>images/xp_plus.gif</xsl:variable>
	<xsl:variable name="MINUS_IMAGE">
		<xsl:value-of select="$basePath"/>images/xp_minus.gif</xsl:variable>
	<xsl:variable name="MENU_IMAGE">
		<xsl:value-of select="$basePath"/>images/arrow.gif</xsl:variable>
	...
	<xsl:template name="element">
		...
		<xsl:param name="optionality" select="'optional'"/>
		<!-- 'mandatory' of 'optional' of leeg (= mandatory). Mandatory wordt altijd opgeslagen -->
		...
		<div class="{$class}">
			...
			<xsl:element name="span">
				<xsl:attribute name="fullPath"><xsl:call-template name="full-path"><xsl:with-param name="theParmNodes" select="$path"/></xsl:call-template></xsl:attribute>
			...
			<xsl:element name="div">
				...
				<xsl:choose>
					...
					<xsl:otherwise>
						...
						<p class="error">local-name() = <xsl:value-of select="local-name()"/>
						</p>
						<p class="error">name() = <xsl:value-of select="name()"/>
						</p>
					</xsl:otherwise>
				</xsl:choose>
			</xsl:element>
