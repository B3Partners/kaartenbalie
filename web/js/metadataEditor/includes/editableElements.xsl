...
<xsl:stylesheet 					
    ...
    <xsl:variable name="SELF_REF">this
    </xsl:variable>

	<!-- default text for elements with no default value specified -->
    <xsl:variable name="GLOBAL_DEFAULT">Klik om te bewerken.
    </xsl:variable>

	<!-- add/delete elements/sections menu text -->
    <xsl:variable name="ADD_ELEMENT_ABOVE_TEXT">Voeg element hierboven toe
    </xsl:variable>
    <xsl:variable name="ADD_ELEMENT_BELOW_TEXT">Voeg element hieronder toe
    </xsl:variable>
    <xsl:variable name="DELETE_ELEMENT_TEXT">Verwijder dit element
    </xsl:variable>
    <xsl:variable name="ADD_SECTION_ABOVE_TEXT">Voeg sectie hierboven toe
    </xsl:variable>
    <xsl:variable name="ADD_SECTION_BELOW_TEXT">Voeg sectie hieronder toe
    </xsl:variable>
    <xsl:variable name="DELETE_SECTION_TEXT">Verwijder deze sectie
    </xsl:variable>
    <xsl:variable name="ADD_CHILD_TEXT">Voeg nieuw kind toe
    </xsl:variable>
	
	<!-- expand/collapse section and menu image paths -->
    <xsl:variable name="EXPAND_TEXT">Klap deze sectie in/uit
    </xsl:variable>
    <xsl:variable name="PLUS_IMAGE">
        <xsl:value-of select="$basePath"/>images/xp_plus.gif
    </xsl:variable>
    <xsl:variable name="MINUS_IMAGE">
        <xsl:value-of select="$basePath"/>images/xp_minus.gif
    </xsl:variable>
    <xsl:variable name="MENU_IMAGE">
        <xsl:value-of select="$basePath"/>images/arrow.gif
    </xsl:variable>
    <xsl:variable name="MENU_TOOLTIP">Klik om opties te zien
    </xsl:variable>
    ...
    <xsl:template name="element">
        ...
        <div class="{$class}">
            ...
            <xsl:element name="span">
                ...
                <xsl:attribute name="onclick">startEdit(event)
                </xsl:attribute>
                <xsl:if test="$picklist != ''">
                    <xsl:attribute name="picklist">
                ...
                <xsl:choose>
                    ...
                    <xsl:when test="normalize-space($path)">
                        <xsl:attribute name="class">unchanged-value
                        </xsl:attribute>
                    ...
                    <xsl:otherwise>
                        ...
                        <xsl:attribute name="class">default-value
                        </xsl:attribute>
                        <xsl:attribute name="changed">false
                        </xsl:attribute>
                        <xsl:choose>
    ...
    <xsl:template name="section">
        ...
        <div class="section" section-path="{$section-path}" id="{$section-path}">
            ...
            <xsl:element name="div">
                <xsl:attribute name="class">section-content
                </xsl:attribute>
				<!-- start collapsed? -->
                ...
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        ...
                        <h2 class="error">Transform error: Local name is blank
                        </h2>
                        <p class="error">local-name() =
            ...
            <xsl:choose>
                <xsl:when test="$expandable = 'true'">
                    <a href="javascript:void(0)" onclick="expandNode(this);return false;" class="expandable" title="{$EXPAND_TEXT}">
                        ...
                        <span class="section-title" onmouseover="sectionTitleHover(this);">
                            <xsl:value-of select="$title"/>
                        </span>
                    </a>
                </xsl:when>
    ...
    <xsl:template name="repeatable-menu">
        ...
        <xsl:variable name="jsFunction">
            <xsl:choose>
                ...
                <xsl:otherwise>
                    <h2 class="error">Argument 'type' not valid. It must be either 'element' or 'section' (both with quotes)
                    </h2>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
        ...
        <span onclick="showMenu(event);">
            <xsl:element name="img">
                ...
                <xsl:attribute name="onmouseover">menuImgHover(this);
                </xsl:attribute>
                <xsl:attribute name="class">
                    <xsl:value-of select="$menu-img-class"/>
                </xsl:attribute>
            </xsl:element>
            <ul class="menu" style="display: none;">
                <li class="menuaddabove">
                    <xsl:element name="a">
                        ...
                        <xsl:attribute name="onclick">add
                            <xsl:value-of select="$jsFunction"/>(
                            <xsl:value-of select="$SELF_REF"/>,'
                            <xsl:value-of select="$path"/>',true)
                        </xsl:attribute>
                        <xsl:choose>
                            <xsl:when test="$type = 'element'">
                ...
                <li class="menuaddbelow">
                    <xsl:element name="a">
                        ...
                        <xsl:attribute name="onclick">add
                            <xsl:value-of select="$jsFunction"/>(
                            <xsl:value-of select="$SELF_REF"/>,'
                            <xsl:value-of select="$path"/>',false)
                        </xsl:attribute>
                        <xsl:choose>
                            <xsl:when test="$type = 'element'">
                ...
                <li class="menudelete">
                    <xsl:element name="a">
                        ...
                        <xsl:attribute name="onclick">delete
                            <xsl:value-of select="$jsFunction"/>(
                            <xsl:value-of select="$SELF_REF"/>,'
    ...
    <xsl:template name="anchor">
        ...
        <xsl:param name="name-shown">no name
        </xsl:param>
        <xsl:param name="target">viewer
        </xsl:param>
        <xsl:element name="a">
