...
<xsl:stylesheet
	...
	Properties in deze stylesheet kunnen NIET gemakkelijk van plaats veranderen binnen een tab of op een andere tab geplaatst worden.
	(Dit heeft te maken met het feit dat in "fullXML" metaEditType modus (zie commentaar bij body-tag) alle xml-tags in volgorde van voorkomen
	in de resulterende html-file worden opgeslagen door NPS. )
	Titel van een property (waarde/xml-node/xml-tag) kan makkelijk aangepast worden per template.
	-->
...
<!-- 
	<xsl:template match="/">
		<xsl:call-template name="metadataStart"/>
	</xsl:template>
-->
	...
	<!-- Bepaalt de achtergrond voor het xhtml-resultaat aan de hand van de inhoud van een node -->
	<xsl:template name="get-background-class-name">
		<xsl:choose>
          <xsl:when test="normalize-space(/metadata/dataset_description/geokey_database/description) = 'basisdata Nederland' ">
			  <xsl:text>achtergrond-nederland</xsl:text>
          </xsl:when>
          <xsl:when test="normalize-space(/metadata/dataset_description/geokey_database/description) = 'basisdata Europa' ">
 			  <xsl:text>achtergrond-europa</xsl:text>
         </xsl:when>
          <xsl:when test="normalize-space(/metadata/dataset_description/geokey_database/description) = 'basisdata Wereld' ">
 			  <xsl:text>achtergrond-wereld</xsl:text>
         </xsl:when>
          <xsl:when test="normalize-space(/metadata/dataset_description/geokey_database/description) = 'projectdata' ">
			  <xsl:text>achtergrond-project</xsl:text>
          </xsl:when>
          <xsl:when test="normalize-space(/metadata/dataset_description/geokey_database/description) = 'webservice' ">
			  <xsl:text>achtergrond-webservice</xsl:text>
          </xsl:when>
          <xsl:otherwise>
			  <xsl:text/>
          </xsl:otherwise>
	...
	<xsl:template name="overzicht-tab">
		<div id="overzicht" class="tab-definition" style="display:block">
			<xsl:apply-templates select="/metadata/dataset_description/identification/dataset_title"/>
			<xsl:apply-templates select="/metadata/dataset_description/overview/summary"/>
			<xsl:apply-templates select="/metadata/dataset_description/overview/producer_organisation_name"/>			
			<xsl:apply-templates select="/metadata/dataset_description/administrative_metadata"/>
		</div>
	</xsl:template>
	...
	<xsl:template match="metadata/dataset_description/identification/dataset_title">
	...
	<xsl:template match="producer_organisation_name">
		<xsl:call-template name="edit_element">
	...
	<xsl:template match="metadata/dataset_description/administrative_metadata">
		<xsl:apply-templates select="point_of_contacts_role"/>
		<xsl:call-template name="separator"/>
		<xsl:apply-templates select="organisation_role"/>
		<xsl:call-template name="separator"/>
